...
//ThresholdPanGesture类是一个UIPanGestureRecognizer子类，它提供一种延迟手势识别器效果的方法，直到正在进行的手势通过指定的移动阀值。示例代码的touchesMoved(with:)方法使用此类让用户在拖拽对象之间平滑过渡并在单个双指手势中旋转对象：
/**
...
class ThresholdPanGesture: UIPanGestureRecognizer {
    ...
    /// - Tag: touchesMoved
    //touchesMoved(with:)方法使用此类让用户在拖拽对象之间平滑过渡并在单个双指手势中旋转对象：
